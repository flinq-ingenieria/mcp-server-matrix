...
| Инструмент | Описание |
|------------|----------|
| `send_message` | Отправить сообщение (текст, HTML или Markdown) с поддержкой ответов |
| `read_messages` | Прочитать последние сообщения (с пагинацией) |
| `get_messages_by_date` | Получить сообщения за период (по датам) |
| `list_rooms` | Список всех комнат пользователя |
...
| `get_notification_counts` | Непрочитанные сообщения и упоминания по комнатам |
| `get_direct_messages` | Список личных переписок с непрочитанными |
| `send_direct_message` | Отправить ЛС (автоматически создаёт комнату) |
| `get_user_profile` | Профиль пользователя — имя, аватар |
